...
public class ThirdPrefResult extends RecyclerView.Adapter<ThirdPrefResult.ViewHolder>{
    private Context mContext;
    private List<ResultData> third_pref_list;
    private DatabaseReference UsersRef;


    public ThirdPrefResult(Context mContext, List<ResultData> second_pref_list){
        this.mContext = mContext;
        this.third_pref_list = second_pref_list;
    }


    @NonNull
    public ThirdPrefResult.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.user_item,parent,false);
        UsersRef = FirebaseDatabase.getInstance().getReference().child("Users");
        return new ThirdPrefResult.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final ThirdPrefResult.ViewHolder holder, int position) {

        final ResultData user = third_pref_list.get(position);
